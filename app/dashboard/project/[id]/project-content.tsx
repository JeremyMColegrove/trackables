...
export function ProjectContentSkeleton() {
  return (
    <div className="mx-auto flex w-full min-w-0 max-w-7xl flex-1 space-y-6 px-4 md:px-6">
      <div className="flex min-w-0 flex-1 flex-col gap-4">
        <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
          <Skeleton className="h-4 w-40" />
        </div>

        <div className="mb-4 mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <CardContent className="min-w-0">
                  <div className="grid min-w-0 gap-6 xl:grid-cols-2 xl:items-start">
                    <FormSubmissionsTable
                      data={project.recentSubmissions}
                      isDisabled={!(project.settings?.isFormEnabled ?? true)}
                    />
                    <UsageEventsTable
                      data={project.recentUsageEvents}
                      isDisabled={!(project.settings?.isApiEnabled ?? true)}
                    />
                  </div>
                </CardContent>
              </Card>
