...
export async function recordApiUsage(input: RecordApiUsageInput) {
  ...
  if (apiKey.expiresAt && apiKey.expiresAt <= now) {
    logger.warn(
      { apiKeyId: apiKey.id, trackableId: project?.id },
  ...
  if (!project || project.archivedAt) {
    logger.warn(
      { apiKeyId: apiKey.id, trackableId: apiKey.projectId },
      "Trackable not found or archived."
    )
    throw new TRPCError({
      code: "NOT_FOUND",
  ...
  if (project.kind !== "api_ingestion") {
    logger.warn(
      { apiKeyId: apiKey.id, trackableId: project.id, kind: project.kind },
      "Trackable does not accept log events."
    )
    throw new TRPCError({
      code: "PRECONDITION_FAILED",
  ...
  try {
    ...
    logger.warn(
      ...
      "Quota exceeded for logging API usage."
    )
    throw err
  }
